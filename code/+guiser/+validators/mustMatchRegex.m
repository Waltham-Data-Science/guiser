...
%
%   Error Conditions:
%       - Throws 'guiser.validators:mustMatchRegex:InvalidInputType' if VALUE
%         is not a character row vector or string scalar.
%       - Throws 'guiser.validators:mustMatchRegex:NoMatch' if VALUE does not
%         match the PATTERN.
%
%   Example Usage in arguments block:
%       arguments
%           inputCode (1,:) char {mustBeNonempty, guiser.validators.mustMatchRegex(inputCode, '^[A-Z]{3}\d{5}$')}
%       end
%
...
if ~( (ischar(value) && isrow(value)) || (isstring(value) && isscalar(value)) )
    error('guiser.validators:mustMatchRegex:InvalidInputType', ...
...
if ~( (ischar(pattern) && isrow(pattern)) || (isstring(pattern) && isscalar(pattern)) )
     error('guiser.validators:mustMatchRegex:InvalidPatternType', ...
...
if isempty(matchResult)
    ...
    error('guiser.validators:mustMatchRegex:NoMatch', ...
